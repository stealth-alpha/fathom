...
- **`fathom activate <license-key>`** — verifies a Fathom Pro license via the
  Gumroad license API and stores the result in
  `~/.config/fathom/license.json`. No features are gated yet; Pro features
  ship in an upcoming release.
- **License tier in `--version`** — the version output now prints the current
  tier (`free` or `pro` with a masked key).

### Fixed

- Landing page Pro price corrected from $9 to $19 to match the actual
  Fathom Pro product; checkout button now points to the Fathom Pro Gumroad
  listing, footer links to the Launch Playbook ebook.
